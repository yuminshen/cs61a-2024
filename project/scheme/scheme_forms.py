...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        lambda_helper = LambdaProcedure(signature.rest, expressions.rest, env)
        env.define(signature.first, lambda_helper)
...
def make_let_frame(bindings, env):
    ...
    while(temp != nil):
        block = temp.first
        validate_form(block, 2, 2)
        name = block.first
        val = eval_all(block.rest, env)
