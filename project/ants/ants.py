    ...
    def nearest_bee(self):
        ...
        while True:
            if tmp_place.is_hive:
                return None
            if tmp_place.bees and self.lower_bound <= count \
                    and count <= self.upper_bound :
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        victim_bees = self.place.bees
        super().reduce_health(amount)

        if self.health <= 0 :
            for victim in list(victim_bees):
                victim.reduce_health(self.damage+amount)
        else :
            for victim in list(victim_bees):
                victim.reduce_health(amount)
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt protects the ants behind it"""
...
# The HungryAnt Class
class HungryAnt(Ant):
    """HungryAnt eats a bee in its place and chew it for next 3 truns"""
...
class TankAnt(ContainerAnt):
    """TankAnt provides protection to other Ants and does 1 harm to all the bees in its place each turn."""
...
class ScubaThrower(ThrowerAnt):
    """Scuba is an ant that can be placed in water"""
