...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    selectable = [str for str in paragraphs if select(str) is True]
    if k >= len(selectable) : return ''
    else : return selectable[k]
    # END PROBLEM 1
...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def select_about(str):
        str2list = split(lower(remove_punctuation(str)))
        for a in str2list :
           if a in subject : return True
        return False
    return select_about
...
def accuracy(typed, source):
    ...
    for i in range(typed_len):
        if i<source_len and typed_words[i]==source_words[i]:
            matched+=1
    ratio = 100.0*matched/typed_len
    return ratio
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list :
        ...
        word, minimum = word_list[0], limit+1
        for p in word_list:
            temp = diff_function(typed_word, p, limit)
            if temp < minimum :
                word, minimum = p, temp
        if minimum > limit :
            word = typed_word
...
s
    ...
    """
    # BEGIN PROBLEM 6
    def helper(typed, source, curr):
        if curr > limit :
            return limit+1
        elif not typed or not source :
            return abs(len(typed)-len(source))
        elif typed[0]==source[0] :
            return helper(typed[1:], source[1:], curr)
        else :
            return helper(typed[1:], source[1:], curr+1)+1
